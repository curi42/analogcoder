...
## 스펙에 없던 결정 하나 (구현 전 확인 필요)

설계 문서는 **면적 단계의 가드 밴드**를 다루지 않았다. 이 계획은 다음과 같이 정했다:

> 면적 단계는 비율 가드(`guard_band`)를 갖지 않는다. 실측 코너 여유분이 있는 기준은 그것으로 보호되고, **없는 기준은 여유분 0으로 판정된다** — 즉 "통과하기만 하면 된다". 그 이름들을 `unguarded_criteria`로 기록한다.

이것은 **안전을 느슨하게 하는 쪽**이다. `optimize:` 선언이 없는 스펙에서는 모든 기준이 무방비가 된다. 대안은 (a) 면적 단계에 별도 `guard_band` 선언을 요구한다 — 그러면 "선언 없이 자동으로 돈다"가 깨진다, (b) 코너 스윕이 없으면 면적 단계를 아예 안 돌린다 — 그러면 지금 벤치마크 대부분에서 안 돈다. 셋 중 무엇을 고를지는 구현 착수 전에 확정한다.
