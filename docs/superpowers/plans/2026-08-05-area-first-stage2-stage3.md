...
> **결정 (2026-08-05, 승인됨):** 권고안을 채택한다. 선별용 시뮬레이션은
> 시뮬레이터 에이전트를 거치지 않고, 승자만 오늘의 `agents.simulate` 경로를
> 탄다. **설계 스펙 본문은 고치지 않는다** — 스펙은 그 시점의 설계 기록이고,
> 이 결정은 계획이 실행 전에 발견한 결함에 대한 것이다. 이 결정으로 생기는
> 새 위험 하나를 §Task 5에 적어 둔다: 선별과 판정이 **서로 다른 경로**로
> 측정하게 되므로, 둘이 어긋나면 "선별이 고른 승자가 판정에서 다른 값을
> 낸다"가 조용히 일어날 수 있다. Task 5가 그 어긋남을 계측한다.
