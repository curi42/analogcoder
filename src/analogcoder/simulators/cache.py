...
#
# 프로덕션 PDK 의 코너 체인은 확인된 바로 **최소 3단**이다: 덱 -> 코너 지정
# 파일(축마다 `.lib` 호출 한 줄) -> 축 라이브러리 섹션(그 안에 다시 약 10줄의
# 참조) -> 모델/스큐 파일. 즉 **코너 하나가 닿는 파일이 10개를 넘는데 이 값이
# 1 인 한 지문에 드는 것은 그중 하나(코너 지정 파일)뿐이다** - 그 파일이
# 바뀌면 미적중이지만 축 라이브러리나 모델 파일만 바뀌면 적중한다.
#
# 한 겹 더 따라가지 않기로 한 근거 넷:
# 1. **깊이 2 로 늘려도 구멍이 안 닫힌다.** 3단의 약 10개 파일이 여전히
#    빠지고, 남는 것은 "지문이 PDK 를 본다"는 인상뿐이다 - 이 저장소가 아홉 번
#    값을 치른 모양이 정확히 그것이다. 부분 커버리지는 무커버리지보다 위험하다.
# 2. 깊이 N 의 대상을 *찾으려면* 깊이 N-1 의 파일을 **읽어야** 한다. 지문이
#    파일을 읽지 않는 것은 이 모듈의 설계 결정이고("PDK 모델 파일은 수십
#    MB이고, 시뮬레이션마다 읽으면 캐시가 아끼려던 것을 도로 쓴다"), 최상위
#    `.include` 가 그 수십 MB 파일을 **직접** 가리키는 덱이 실제로 있다.
#    `simulation_key` 는 `run()` 마다 돈다.
# 3. 어느 깊이를 골라도 그 수는 "체인이 거기서 끝난다"는 미확인 가정이다.
#    확인 안 된 수를 고르는 것은 이 저장소가 금지하는 추측이다.
# 4. 벤치마크 20개 덱의 캐시 키가 바뀐다(`pdk_corner.inc` 가 sky130 모델
#    파일들을 다시 include 한다).
#
...
# 를 함께 고쳐야 한다. 제대로 닫으려면 필요한 것은 깊이 +1 이 아니라
# **코너 체인 전체의 지문**이고, 그것은 별도 설계다.
